...
def pair_analysis(filename,cutname):		
	...
	rootfile.ls();
	rootdir = rootfile.Get("analysis-event-mixing");
	rootdir.ls();
	rootdir.cd();
	list_mix = rootdir.Get("output");
	list_mix.ls();
	...
	for ip in range(0,np):
		list2 = list_mix.FindObject("PairsBarrelME{0}_lmee_{1}".format(parnames[ip],cutname));
		list2.Print();
		hMass[ip] = list2.FindObject("Mass");
		hPt[ip] = list2.FindObject("Pt");
	...
	rootdir_se = rootfile.Get("analysis-same-event-pairing");
	rootdir_se.ls();
	rootdir_se.cd();
	list_se = rootdir_se.Get("output");
	list_se.ls();
	...
	for ip in range(0,np):
		list_se2 = list_se.FindObject("PairsBarrelSE{0}_lmee_{1}".format(parnames[ip],cutname));
		hMassSE[ip] = list_se2.FindObject("Mass");
		hPtSE[ip] = list_se2.FindObject("Pt");
		...
		hMass_PtSE[ip] = list_se2.FindObject("Mass_Pt");
